...
사용자가 직접 실행해 제공한 화면입니다. `결과.docx`에 담긴 캡처와 기존 실행 기록을 사용했으며, 이번 README 갱신에서 앱을 재실행하거나 답변을 편집하지 않았습니다. **답변 생성 예시와 보완이 필요한 사례를 함께 공개합니다.** 화면은 작동 관찰 자료이며, 답변·인용 정확성이나 임상적 안전성을 검증한 결과가 아닙니다.

| 사례 | 화면의 처리 결과 | 확인할 내용 |
|---|---|---|
| 우울증 선별 도구에 관한 일반 질문 | `ANSWER_DRAFT` | 답변 초안과 모델에 제공한 출처 표시 |
| 질문을 입력하지 않은 경우 | `EMPTY_INPUT` | 답변 생성 대신 질문 입력 안내 |
| 개인 두통 증상에 관한 질문 | `ANSWER_DRAFT` | 개인 증상 질문이 일반 생성 경로로 통과한 분기 누락 |
| 혈액형에 따른 약물 선택 질문 | `ANSWER_DRAFT` | 근거 부족을 언급한 뒤에도 설명을 이어간 답변 보류 한계 |

<details open>
<summary>1. 일반 질문 — 답변 초안과 출처 표시</summary>

질문: **“일차 의료에서 우울증 선별에 사용할 수 있는 도구는 무엇인가?”**

진료지침에서 검색한 본문을 모델에 제공하고, 반환된 답변 초안과 참고 자료의 문서명·PDF 페이지·청크 ID를 표시한 예시입니다. 출처 목록은 모델에 제공한 자료이며, 답변의 각 주장과 인용이 일치한다고 자동 검증한 것은 아닙니다.

**질문 입력 및 처리 결과**

![우울증 선별 도구 질문 입력 후 ANSWER_DRAFT가 표시된 화면](docs/images/general-question-input.png)

**답변 초안 및 모델에 제공한 출처**

![우울증 선별 도구에 대한 답변 초안과 PDF 페이지 및 청크 ID를 표시한 화면](docs/images/general-question-answer.png)

</details>

<details>
<summary>2. 빈 입력 — 질문 입력 안내</summary>

질문을 입력하지 않고 실행한 경우 `EMPTY_INPUT`으로 분기합니다. 검색·LLM 호출 없이 “질문을 입력해 주세요.”라는 고정 안내를 표시합니다. 입력창의 회색 예문은 실제로 제출한 질문이 아니라 입력을 돕는 예시 문구입니다.

![빈 질문 실행 시 EMPTY_INPUT과 질문 입력 안내가 표시된 화면](docs/images/empty-input.png)

</details>

<details>
<summary>3. 개인 증상 질문 — 입력 분기 누락 사례</summary>

질문: **“내가 머리가 좀 아픈데 어떤 병과 관련되어있니?”**

개인 증상에 대한 질환 추정 요청이 고정 안내로 분기하지 않고 일반 RAG 경로를 통과했습니다. 답변에서 자료 범위의 한계를 언급했으나, 상태는 `ANSWER_DRAFT`이며 추가 설명을 생성했습니다. **개인 증상 질문의 입력 분기와 답변 보류를 보완해야 하는 사례**로 기록합니다.

![개인 두통 질문이 ANSWER_DRAFT로 처리되어 설명을 계속 생성한 분기 누락 화면](docs/images/clinical-symptom-question.png)

</details>

<details>
<summary>4. 근거 부족 질문 — 답변을 완전히 보류하지 못한 사례</summary>

질문: **“혈액형에 따라 항우울제를 다르게 선택해야 하는가?”**

모델이 제공된 자료에서 질문의 근거를 찾지 못했다고 언급한 뒤에도 다른 약물 선택 요인에 관한 설명을 이어갔습니다. `NO_EVIDENCE`로 전환하지 않고 `ANSWER_DRAFT`로 표시된 **불완전한 답변 보류 사례**입니다.

![혈액형 질문에 근거 부족을 언급한 뒤 설명을 계속한 실제 실행 화면](docs/images/evidence-limitation.png)

</details>

개인정보 입력 시 `PII_BLOCK`으로 분기한 캡처도 확인했지만, 이메일 주소가 보여 공개 갤러리에서는 제외했습니다. 등록한 정규식에 해당하는 일부 개인정보 형태만 감지하며, 개인정보 차단의 완전성을 보장하지 않습니다.

위 화면은 기존 실행 기록입니다. 다른 실행에서는 답변 표현과 인용이 달라질 수 있으며, 새로 캡처하지 않은 처리 상태까지 검증한 것으로 해석하지 않습니다.
...
PDF 원문, 로컬 Qdrant DB, 가상환경, 모델 캐시, API 키와 원문을 포함한 평가 결과는 저장소에서 제외합니다. 로컬 참고 문서 `결과.docx`도 업로드 대상에서 제외합니다. 화면은 사용자가 제공한 기존 데모 캡처와 `결과.docx`에서 추출한 이미지만 사용했고, 원본 이미지의 답변 내용을 편집하지 않았습니다. 이메일 주소가 포함된 캡처는 업로드하지 않았습니다.
